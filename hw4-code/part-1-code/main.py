...
torch.backends.cudnn.deterministic = False  # Allow cuDNN to optimize
...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    # Enable mixed precision training for faster training and better GPU utilization
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int, default=32)  # Increased from 8 for better GPU utilization
    parser.add_argument("--num_workers", type=int, default=4)  # Parallel data loading
    ...
    # Create dataloaders for iterating over the dataset with optimizations
    ...
    if args.debug_train:
        train_dataloader = DataLoader(small_train_dataset, shuffle=True, **dataloader_kwargs)
        eval_dataloader = DataLoader(small_eval_dataset, **dataloader_kwargs)
        print(f"Debug training...")
        print(f"len(train_dataloader): {len(train_dataloader)}")
        print(f"len(eval_dataloader): {len(eval_dataloader)}")
    else:
        train_dataloader = DataLoader(tokenized_dataset["train"], shuffle=True, **dataloader_kwargs)
        eval_dataloader = DataLoader(tokenized_dataset["test"], **dataloader_kwargs)
        print(f"Actual training...")
        print(f"len(train_dataloader): {len(train_dataloader)}")
        print(f"len(eval_dataloader): {len(eval_dataloader)}")
        print(f"Batch size: {args.batch_size}, Workers: {args.num_workers}, Mixed Precision: {torch.cuda.is_available()}")

    # Train model on the original training dataset
    if args.train:
        model = AutoModelForSequenceClassification.from_pretrained("bert-base-cased", num_labels=2)
        model.to(device)
        # Compile model for PyTorch 2.0+ for additional speedup (if available)
